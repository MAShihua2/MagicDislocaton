...
layerize_direct = np.array([-0.9223,   1.3043, 0])
move_v1(path=input_file,
        lattice=3.195,
        x_direct=np.array([1,1,1]),
        y_direct=np.array([1, 1, -2]),
        z_direct=np.array([-1, 1, 0]),
        burgers_vector=burgers_vector,
        layerize_direct=layerize_direct,
        target_plane=layerize_direct,
        insert_plane=np.array([1, 0, 0]),
        scale=5,
        base=2,
        num_points=48)